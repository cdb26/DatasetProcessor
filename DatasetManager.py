...
Dataset Manager  —  optimised rewrite
  ...
  • DELETE image with automatic sequence rebalancing
    - Deletes color (.jpg) and/or depth (.png) on disk
    - Removes the record from the in-memory match list
    - Renames every subsequent file in the same group (room/height/angle/
      distance/lighting) so sequences stay gapless, ascending order so we
      never clobber a rename target that hasn't been moved yet
    - Updates Treeview rows in-place (single-row refresh, no full re-render)
    - Cleans up object_counts for the deleted key
    - Keyboard shortcut: <Delete> key in viewer
"""
...
def group_key_from_parts(p: dict) -> str:
    """5-field group identifier — no sequence, no ext."""
...
class DatasetManagerApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkEntry(path_row, textvariable=self.dataset_path,
                     ...
                     text_color=TEXT_MAIN, border_color=BORDER, width=560).pack(side="left", pady=14)
    ...
    def _build_rename_tab(self, parent):
        ...
        ctk.CTkButton(right, text="⟳  Preview Changes", height=38, fg_color=BG_FIELD,
        ...
        ctk.CTkButton(right, text="✔  Apply Rename", height=44, fg_color=ACCENT,
                      hover_color=ACCENT2, font=("Courier New", 13, "bold"),
                      text_color="white", command=self._apply_rename).pack(fill="x", pady=6)
    ...
    def _build_move_tab(self, parent):
        ...
        ctk.CTkLabel(rc1, text="Room code (FFRRRR):", font=("Courier New", 10),
                     text_color=TEXT_DIM).pack(side="left", padx=(0,10))
        self._move_room_entry = ctk.CTkEntry(rc1, width=140, font=("Courier New", 12, "bold"),
                                             fg_color=BG_CARD, text_color=TEXT_MAIN,
                                             border_color=ACCENT, placeholder_text="e.g. 070701")
        ...
        ctk.CTkButton(left, text="✕  Clear all rooms", width=140, fg_color=BG_FIELD,
    ...
    def _clear_move_rooms(self):
        self._move_rooms.clear(); self._refresh_rooms_list()
    ...
    def _gather_move_plan(self) -> list[tuple[str, str]] | None:
        ...
        if not dest:
            messagebox.showerror("Error", "Please select a destination folder."); return None
        if not self._move_rooms:
            messagebox.showerror("Error", "No rooms selected."); return None
    ...
    def _preview_move(self):
        ...
        if not plan:
            self._log(self._move_log, "No matching files found for selected rooms."); return
    ...
    def _execute_move(self):
        ...
        if plan is None: return
        if not plan:
            messagebox.showinfo("Nothing to do", "No matching files found."); return
        verb = "copy" if self._move_copy.get() else "move"
        jpg  = sum(1 for s, _ in plan if s.lower().endswith(".jpg"))
    ...
    def _apply_rename(self):
        plan = self._gather_rename_plan()
        if not plan:
            messagebox.showinfo("Nothing to do", "No matching files to rename."); return
        ...
        for idx, rec in enumerate(matches):
            ...
            tree.insert("", "end",
                        values=(rec["base_key"], p["room"], p["height"],
                                ...
                                p["distance"], LIGHT_LABEL.get(p["lighting"], p["lighting"]),
                                p["sequence"], has_txt),
    ...
    def _refresh_tree_row(self, idx: int):
        """Update exactly one Treeview row without touching others."""
        children = self._filter_tree.get_children()
        if idx >= len(children): return
        rec     = self._filter_matches[idx]
        p       = rec["parts"]
        has_txt = "+".join(("JPG",)*bool(rec["color_path"]) + ("PNG",)*bool(rec["depth_path"]))
    ...
    # ══════════════════════════════════════════════════════════════════════════
    @staticmethod
    def _resequence_on_disk(deleted_rec: dict,
                            all_matches:  list[dict],
                            deleted_idx:  int,
                            object_counts: dict) -> list[str]:
        """
        After deleting `deleted_rec`, renumber every later record in the same
        5-field group so sequences are gapless (no holes).

        Traversal is ascending so we never overwrite a rename target before
        it has been moved.  Each rename: seq N → seq N-1.

        Returns human-readable log lines.
        """
        logs: list[str] = []
        deleted_seq = int(deleted_rec["parts"]["sequence"])
        grp         = group_key_from_parts(deleted_rec["parts"])

        # Gather siblings after the deleted record, in ascending order
        to_shift: list[tuple[int, dict]] = [
            (i, rec) for i, rec in enumerate(all_matches)
            if i != deleted_idx
            and group_key_from_parts(rec["parts"]) == grp
            and int(rec["parts"]["sequence"]) > deleted_seq
        ]
        to_shift.sort(key=lambda x: int(x[1]["parts"]["sequence"]))

        for match_idx, rec in to_shift:
            old_seq     = rec["parts"]["sequence"]
            new_seq     = f"{int(old_seq)-1:04d}"
            old_key     = rec["base_key"]
            new_key     = base_key_from_parts({**rec["parts"], "sequence": new_seq})
            rename_ok   = True

            for path_key, is_depth, ext in [("color_path", False, ".jpg"),
                                             ("depth_path", True,  ".png")]:
                old_path = rec[path_key]
                if not old_path or not os.path.isfile(old_path): continue
                new_name = build_filename({**rec["parts"], "sequence": new_seq,
                                           "is_depth": is_depth, "ext": ext})
                new_path = os.path.join(os.path.dirname(old_path), new_name)
                try:
                    os.rename(old_path, new_path)
                    rec[path_key] = new_path
                    logs.append(f"  ↳ {os.path.basename(old_path)}  →  {new_name}")
                except Exception as e:
                    logs.append(f"  ✘ {os.path.basename(old_path)}: {e}")
                    rename_ok = False

            if rename_ok:
                rec["parts"]["sequence"] = new_seq
                rec["base_key"]          = new_key
                if old_key in object_counts:
                    object_counts[new_key] = object_counts.pop(old_key)
    ...
    def _open_image_picker(self, start_idx: int):
        ...
        ctk.CTkLabel(rp, text="OBJECT  COUNTER",
                     font=("Courier New",9,"bold"), text_color=ACCENT).pack(anchor="w", padx=14, pady=(12,4))
        ...
        def _add_obj_row(name: str, counts: dict):
            ...
            ctk.CTkLabel(row, text=name, font=("Courier New",10),
                         text_color=TEXT_MAIN, anchor="w", width=90).pack(side="left", padx=(8,4), pady=6)
            cnt_var = tk.StringVar(value=str(counts.get(name,0)))
            obj_count_vars[name] = cnt_var
            for txt, hov, delta_or_cmd in [
                ("−", DANGER,  lambda n=name: _change_count(n,-1)),
                ("+", SUCCESS, lambda n=name: _change_count(n,+1)),
            ]:
                ctk.CTkButton(row, text=txt, width=26, height=26, fg_color=BG_FIELD,
                              hover_color=hov, font=("Courier New",13,"bold"),
                              text_color=TEXT_MAIN, corner_radius=4,
                              command=delta_or_cmd).pack(side="left" if txt=="−" else "left", padx=1)
                if txt == "−":
        ...
        def _edit_obj_name(old_name: str):
            ...
            ent = ctk.CTkEntry(dlg, textvariable=name_var, width=240, font=("Courier New",12),
            ...
            def _apply():
                ...
                if not new_name:         err_var.set("Name cannot be empty."); return
                if new_name==old_name:   dlg.destroy(); return
                if new_name in _all_known_objects(): err_var.set(f"'{new_name}' already exists."); return
        ...
        ctk.CTkButton(nav, text="💾  Save Record", width=150, height=36,
                      fg_color=SUCCESS, hover_color="#2AB87A",
                      font=("Courier New",11,"bold"), text_color="white",
                      command=lambda: _save_excel()).pack(side="left", padx=10)
        ctk.CTkButton(nav, text="✎  Rename", width=110, height=36,
                      fg_color=WARNING, hover_color="#D4901E",
                      font=("Courier New",11,"bold"), text_color="white",
                      command=lambda: _rename_dialog()).pack(side="left", padx=4)

        # ─── DELETE BUTTON ────────────────────────────────────────────────────
        ctk.CTkButton(nav, text="🗑  Delete", width=110, height=36,
        ...
        def _delete_dialog():
            ...
            if not has_color and not has_depth:
                messagebox.showinfo("Nothing to delete",
                                    "No files on disk for this record.", parent=win); return

            grp         = group_key_from_parts(p)
            deleted_seq = int(p["sequence"])
            ...
            if siblings > 0:
                ctk.CTkLabel(dlg,
                             text=f"⚠  {siblings} file(s) in this group will be renumbered to close the gap.",
            ...
            of = ctk.CTkFrame(dlg, fg_color=BG_FIELD, corner_radius=8)
            ...
            def _do_delete():
                ...
                if reseq_var.get() and siblings > 0:
                    # Run resequence in background thread, finish on main thread
                    def _worker():
                    ...
                    _finish(fully_deleted, [])

            def _finish(fully_deleted: bool, reseq_logs: list[str]):
                cur_i = state["idx"]

                if fully_deleted:
                    # Remove from memory
                    state["object_counts"].pop(rec["base_key"], None)
                    matches.pop(cur_i)

                    # Remove from Treeview and retag all subsequent rows
                    children = self._filter_tree.get_children()
                    if cur_i < len(children):
                        self._filter_tree.delete(children[cur_i])
                    remaining = self._filter_tree.get_children()
                    for j in range(cur_i, len(remaining)):
                        existing = [t for t in self._filter_tree.item(remaining[j], "tags")
                                    if not t.startswith("idx:") and t not in ("even","odd")]
                        parity = "even" if j%2==0 else "odd"
                        self._filter_tree.item(remaining[j], tags=(*existing, parity, f"idx:{j}"))

                    state["idx"] = min(cur_i, len(matches)-1)
                else:
                    # Partial delete — just refresh this row
                    self._refresh_tree_row(cur_i)

                # Refresh Treeview rows for resequenced siblings (in-place, fast)
                if reseq_logs:
                    for j in range(len(matches)):
                        self._refresh_tree_row(j)

                self._filter_count.set(f"{len(matches)} unique record(s)")
                self._set_status(
                    f"Deleted  ·  {len(reseq_logs)} file(s) resequenced  ·  "
                    f"{len(matches)} record(s) remaining")
                _last_key[0] = None
                refresh(force_obj=True)
        ...
        def _rename_dialog():
            ...
            def _do_rename():
                ...
                if rec["color_path"] and os.path.isfile(rec["color_path"]):
                    np={**nb,"is_depth":False,"ext":".jpg"}
                    np2=os.path.join(os.path.dirname(rec["color_path"]),build_filename(np))
                ...
                if rec["depth_path"] and os.path.isfile(rec["depth_path"]):
                    np={**nb,"is_depth":True,"ext":".png"}
                    np2=os.path.join(os.path.dirname(rec["depth_path"]),build_filename(np))
        ...
        # ── Wire up buttons + keyboard shortcuts ──────────────────────────────
        ...
        win.bind("<Delete>", lambda e: _delete_dialog())   # ← keyboard shortcut
